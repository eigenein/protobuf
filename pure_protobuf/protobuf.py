...
class Flags:
    ...
    PRIMITIVE, EMBEDDED, EMBEDDED_MASK = (
        ...
        8,
        8,
    )  # Used by MessageMetaType to see if a field has embedded definition.
...
# This used to determine the length of unknown tags when loading a message.
...
class MessageType(Type):
    ...
    def dump(self, fp, value):
        if self != value.message_type:
            raise TypeError(
                "Attempting to dump an object with type that's different from "
                "mine."
            )
        for tag, field_type in six.iteritems(self.__tags_to_types):
            if self.__tags_to_names[tag] in value:
                ...
                msg = (
                    "The field with the tag {} is required, "
                ...
                if tag in self.__tags_to_types:
                    ...
                    if not self.__has_flag(
                        ...
                        if wire_type != field_type.WIRE_TYPE:
                            msg = (
                                "The received value with the tag {} "
                                "has incorrect wiretype: {} "
                                "instead of {} expected."
                            ).format(tag, wire_type, field_type.WIRE_TYPE)
                            raise TypeError(msg)
                    elif wire_type != Bytes.WIRE_TYPE:
                        msg = (
                            "Tag %s has wiretype {} while the field is "
                            "packed repeated."
                        ).format(tag, wire_type)
                        raise TypeError(msg)
                    if self.__has_flag(tag, Flags.SINGLE, Flags.REPEATED_MASK):
                        # Single value.
                ...
                for tag, name in six.iteritems(self.__tags_to_names):
                    ...
                    if (has_flag and (name not in message)):
                        if self.__has_flag(
                            ...
                            msg = (
                                "The field with the tag %s ('%s') is required "
                                "but a value is missing."
                            ).format(tag, name)
    ...
    def __restore_type(self, message):
        ...
        for field in message.fields:
            ...
            if field_type not in g:
                msg = "Primitive type {} not found in this protobuf module."
                raise TypeError(msg.format(field_type))
